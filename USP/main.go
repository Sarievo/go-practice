...

	{
		...
		for scanner.Scan() {
			...
			candidates = append(candidates, candidate{
				...
				evaluation: map[string]float64{
					"Physics":     math.Max(scores[4], (scores[0]+scores[2])/2),
					"Chemistry":   math.Max(scores[4], scores[1]),
					"Mathematics": math.Max(scores[4], scores[2]),
					"Engineering": math.Max(scores[4], (scores[2]+scores[3])/2),
					"Biotech":     math.Max(scores[4], (scores[0]+scores[1])/2),
				},
				priorities: data[7:],
			})
		}
